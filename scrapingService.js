...
const NewsScraper = () => {
  ...
  // Sample news data (simulating scraped content)
  const sampleArticles = [
    ...
    {
      id: 4,
      headline: "NASA Announces Discovery of Earth-Like Planet in Nearby Star System",
      author: "Dr. James Patterson",
      publicationDate: "2025-06-07",
      source: "Space Science Daily",
      url: "https://spacenews.com/exoplanet-discovery",
      category: "science",
      summary: "James Webb Space Telescope identifies potentially habitable exoplanet."
    }
  ];
  ...
  // Simulated scraping function (in real implementation, this would use a backend API)
  const scrapeWebsite = async (url) => {
    ...
    try {
      ...
      // which would use Cheerio to scrape the content
  ...
  const filteredAndSortedArticles = articles
    ...
    .sort((a, b) => {
      if (sortBy === 'date') {
        ...
        const aRelevance = filterKeyword ? 
          (a.headline.toLowerCase().match(new RegExp(filterKeyword.toLowerCase(), 'g')) || []).length : 0;
        const bRelevance = filterKeyword ? 
          (b.headline.toLowerCase().match(new RegExp(filterKeyword.toLowerCase(), 'g')) || []).length : 0;
